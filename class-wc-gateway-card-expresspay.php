...
define("CARD_EXPRESSPAY_VERSION", "2.7.1");
	...
	class Wordpress_Card_Expresspay extends WC_Payment_Gateway {
		...
		function init_form_fields() {
			$this->form_fields = array(
				...
				'sign_notify' => array(
					'title'   => __('Использовать цифровую подпись для уведомлений', 'wordpress_card_expresspay'),
					'type'    => 'checkbox',
					'description' => __('Параметр проверки запросов с использование цифровой подписи', 'wordpress_card_expresspay'),
					'desc_tip'    => true
				),
		...
		function receipt_page( $order ){
		  ...
		  function generate_expresspay_form($order_id)
		  {
			...
			$this->log_info('generate_expresspay_form', 'SecretWord - ' . $this->secret_word);
			...
			$this->log_info('generate_expresspay_form', 'Send POST request; ORDER ID - ' . $order_id . '; URL - ' . $add_url . '; TOKEN - ' . $this->token . '; REQUEST - ' . json_encode($request_params));
		...
		private function success($order_id) {
			...
			$html .= '<br/><br/><p class="return-to-shop"><a class="button wc-backward" href="' . get_permalink( woocommerce_get_page_id( "shop" ) ) . '">' . __('Продолжить', 'wordpress_card_expresspay') . '</a></p>';
			...
			if($this->test_mode) : ?>
				<div class="test_mode">
			        <?php _e('Тестовый режим:', 'wc_card_expresspay'); ?> <br/>
    				<input type="button" style="margin: 6px 0;" class="button" id="send_notify_success" value="<?php _e('Отправить уведомление об успешной оплате', 'wordpress_card_expresspay'); ?>" />
			        <input type="button" class="button" style="margin: 6px 0;" id="send_notify_cancel" class="btn btn-primary" value="<?php _e('Отправить уведомление об отмене оплаты', 'wordpress_card_expresspay'); ?>" />
				      ...
				      <script type="text/javascript">
				        jQuery(document).ready(function() {
				          ...
				          function send_notify(type, signature) {
				            jQuery.post('<?php echo get_site_url() . "/?wc-api=wc_card_expresspay&action=notify" ?>', 'Data={"CmdType": ' + type + ', "AccountNo": <?php echo $order->get_order_number(); ?>}&Signature=' + signature, function(data) {alert(data);})
				            .fail(function(data) {
		...
		private function fail($order_id) {
			...
			$html .= '<br/><br/><p class="return-to-shop"><a class="button wc-backward" href="' . $woocommerce->cart->get_checkout_url() . '">' . __('Попробовать заново', 'wordpress_card_expresspay') . '</a></p>';
